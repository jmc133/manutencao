...
public interface FantVisualizarOrcamentosRepository extends JpaRepository<FantVisualizarOrcamentos, Long> {
	...
	@Query(value = "select ROW_NUMBER() OVER(ORDER BY pessoa_juridica.nm_fantasia) Linha, pessoa_juridica.nm_fantasia, pessoa_juridica.cidade, orcamentos.orcador,\r\n"
			+ "defeitos_relatados.descricao, orcamentos.valor_unitario,orcamentos.valor_total, t_veiculos.placa_atual, sum(orcamentos.valor_total)soma,\r\n"
			+ "defeitos_relatados.id_defeitos fk_defeitos, pessoa_juridica.cd_pessoa_jur pj\r\n"
			...
			+ " orcamentos.valor_unitario, orcamentos.valor_total, t_veiculos.placa_atual,fk_defeitos,pj\r\n"
